...
| Avançado | Conteinerização com Docker | Criar Dockerfile para empacotar a aplicação de forma reprodutível. |
| Avançado | Orquestração com docker-compose | Subir API e banco de dados robusto, como PostgreSQL, em ambiente integrado. |
| Avançado | Clean Architecture | Separar entidades, casos de uso e adaptadores para reduzir acoplamento e facilitar evolução. |
...
| Avançado | CI/CD com GitHub Actions | Automatizar testes e lint a cada commit para manter qualidade e integração contínua. |
| Avançado | Documentação com Swagger | Publicar e manter documentação interativa da API, aproveitando suporte nativo do FastAPI. |